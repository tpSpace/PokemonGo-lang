...
func Crawler() {
	...
	// Open or create the pokedex.json file
	file, err := os.OpenFile("pokedex.json", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		fmt.Println("Error: ", err)
		return
	}
	...
	// Initialize an empty array in the JSON file if it's newly created
	...
	for i := 1; i <= 649; i++ {
		...
		// Add comma and newline before each new Pokémon data, except for the first one
		if i > 1 {
			_, err = file.WriteString(",\n")
			if err != nil {
				fmt.Println("Error: ", err)
		...
		// Write the Pokémon JSON data to the file
		_, err = file.Write(pokedexJson)
		if err != nil {
			fmt.Println("Error: ", err)
			return
		}
